...
class cRegexSearcher:
    ...
    ##########################################
        try:
            ...
            #IP Version 4 addresses - 08232011 changed from self.regexes['ipv4address']
            self.regexes['ipv4addr'] = re.compile(r"\b((25[0-4]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[1-9])(\.|\[\.\]|\[dot\]|\(dot\)|\(\.\)|<dot>|<\.>)(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])(\.|\[\.\]|\[dot\]|\(dot\)|\(\.\)|<dot>|<\.>)(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])(\.|\[\.\]|\[dot\]|\(dot\)|\(\.\)|<dot>|<\.>)(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9]))\b", re.I)
                ...
                try:
                    ...
                    for match in rmatches:
                        ...
                        if type(match) == tuple:
                            ...
                            _tmpstring = match
                        try:
                            #Re-fang indicators - best attempt
                            _tmpstring = re.sub(r'[\[\(](([dD][oO][tT])|(\.)|[dD])[\]\)]','.',_tmpstring)
                            _tmpstring = re.sub(r'[\[\(](([aA][tT])|@)[\]\)]','@',_tmpstring)
                            _tmpstring = re.sub(r'[\[\(](([cC][oO][lL][oO][nN])|:)[\]\)]',':',_tmpstring)
                            _tmpstring = re.sub(r'[hH][xX]{2}[pP]://','http://',_tmpstring)
                            _tmpstring = re.sub(r'[hH][xX]{2}[pP][sS]://','https://',_tmpstring)
                            _tmpstring = re.sub(r'[fF][xX][pP]://','ftp://',_tmpstring)
                            _tmpstring = re.sub(r'[fF][xX][pP][sS]://','ftps://',_tmpstring)
                        except:
                            pass
                        ...
                        if not rtype=="url" and not rtype == "filepath" and not rtype == "paste_id":
